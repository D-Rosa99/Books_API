...
test('child should not share bindings of parent set after child creation', async ({
  same,
}) => {
  const instance = pino({ name: 'basicTest', level: 'info' });
  const child = instance.child({});
...
function levelTest(name, level) {
  ...
  test(`passing objects at level ${name}`, async ({ is, same }) => {
    ...
    is(
      new Date(result.time) <= new Date(),
      true,
      'time is greater than Date.now()'
    );
    is(result.pid, pid);
...
test('pino.destination', async ({ same }) => {
  const tmp = join(os.tmpdir(), '_' + Math.random().toString(36).substr(2, 9));
...
test('use `fast-safe-stringify` to avoid circular dependencies', async ({
  deepEqual,
}) => {
  const stream = sink();
  const root = pino(stream);
...
test('throws when setting useOnlyCustomLevels without customLevels', async ({
  is,
  throws,
}) => {
  throws(() => {
