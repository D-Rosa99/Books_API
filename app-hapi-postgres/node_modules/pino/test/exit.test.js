...
test('pino.destination log everything when calling process.exit(0)', async ({
  ...
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'destination-exit.js'),
  ]);
...
test('sync false does not log everything when calling process.exit(0)', async ({
  is,
}) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'syncfalse-exit.js'),
  ]);
...
test('sync false logs everything when calling flushSync', async ({ isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'syncfalse-flush-exit.js'),
  ]);
