...
test('throws if not supplied logger with pino.destination instance with sync false', async ({
  throws,
  doesNotThrow,
}) => {
  throws(() => {
...
test('returns an exit listener function', async ({ is }) => {
  is(
    typeof pino.final(pino(pino.destination({ sync: false })), () => {}),
    'function'
  );
});

test('listener function immediately sync flushes when fired (sync false)', async ({
  pass,
  fail,
}) => {
...
test('listener function immediately sync flushes when fired (sync true)', async ({
  pass,
  fail,
}) => {
...
test('listener function triggers handler function parameter', async ({
  pass,
  fail,
}) => {
  const dest = pino.destination({ dest: getPathToNull(), sync: false });
...
test('passes a specialized final logger instance', async ({
  is,
  isNot,
  error,
}) => {
...
test('returns a specialized final logger instance if no handler is passed', async ({
  is,
  isNot,
}) => {
  const dest = pino.destination({ dest: getPathToNull(), sync: false });
...
test('final logger instances synchronously flush after a log method call', async ({
  pass,
  fail,
  error,
}) => {
