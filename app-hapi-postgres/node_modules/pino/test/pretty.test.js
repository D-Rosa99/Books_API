...
test('can be enabled via exported pino function', async ({ isNot }) => {
  ...
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'basic.js'),
  ]);
...
test('can be enabled via exported pino function with pretty configuration', async ({
  isNot,
}) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'level-first.js'),
  ]);
...
test('can be enabled via exported pino function with prettifier', async ({
  isNot,
}) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'pretty-factory.js'),
  ]);
...
test('throws when prettyPrint is true but pino-pretty module is not installed', async ({
  throws,
  is,
}) => {
...
test('ignores `undefined` from prettifier', async ({ is }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'skipped-output.js'),
  ]);
...
test('parses and outputs chindings', async ({ is, isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'child.js'),
  ]);
...
test('applies formatters', async ({ is, isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'formatters.js'),
  ]);
...
test('parses and outputs chindings with serializer', async ({ is, isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'child-with-serializer.js'),
  ]);
...
test('applies serializers', async ({ is, isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'serializers.js'),
  ]);
...
test('applies redaction rules', async ({ is, isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'redact.js'),
  ]);
...
test('dateformat', async ({ isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'dateformat.js'),
  ]);
...
test('without timestamp', async ({ isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'no-time.js'),
  ]);
...
test('with custom timestamp', async ({ is }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'custom-time.js'),
  ]);
...
test('with custom timestamp label', async ({ is }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'custom-time-label.js'),
  ]);
...
test('errors', async ({ isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'error.js'),
  ]);
  ...
  isNot(
    strip(actual).match(/\(123456 on abcdefghijklmnopqr\): with a message/),
    null
  );
...
test('errors with props', async ({ isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'error-props.js'),
  ]);
...
test('final works with pretty', async ({ isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'final.js'),
  ]);
  ...
  isNot(
    strip(actual).match(
      /WARN\s+\(123456 on abcdefghijklmnopqr\): pino.final with prettyPrint does not support flushing/
    ),
    null
  );
  isNot(
    strip(actual).match(/INFO\s+\(123456 on abcdefghijklmnopqr\): beforeExit/),
    null
  );
});
...
test('final works when returning a logger', async ({ isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'final-return.js'),
  ]);
  ...
  isNot(
    strip(actual).match(/INFO\s+\(123456 on abcdefghijklmnopqr\): after/),
    null
  );
});
...
test('final works without prior logging', async ({ isNot }) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'final-no-log-before.js'),
  ]);
  ...
  isNot(
    strip(actual).match(
      /WARN\s+: pino.final with prettyPrint does not support flushing/
    ),
    null
  );
  isNot(
...
test('works as expected with an object with the msg prop', async ({
  isNot,
}) => {
  var actual = '';
  const child = execa(process.argv[0], [
    join(__dirname, 'fixtures', 'pretty', 'obj-msg-prop.js'),
  ]);
...
test('should not lose stream metadata for streams with `needsMetadataGsym` flag', async ({
  isNot,
}) => {
  const dest = new Writable({
    objectMode: true,
...
test('should not add stream metadata for streams without `needsMetadataGsym` flag', async ({
  is,
}) => {
  const dest = new Writable({
    objectMode: true,
