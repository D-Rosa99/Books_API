...
assign('pid', (data) => data.pid);

assign('tags', (data) => data.tags || []);
...
assign(
  'message',
  (data, colors, field = 'msg') => get(data, field) || data.msg || data.data
);

assign('get', (data, colors, field) => get(data, field));
...
assign('environment', (data, colors, field = 'NODE_ENV') => process.env[field]);

assign('requestId', (data) => data.req && data.req.id);

assign('responseTime', (data) => data.responseTime);
...
assign('payload', (data) => {
  const rawFormat = [Readable, IncomingMessage].find(
    (x) => data.payload instanceof x
  );
...
assign('remotePort', (data) => data.req && data.req.remotePort);

assign('url', (data) => data.req && data.req.url && data.req.url.href);
