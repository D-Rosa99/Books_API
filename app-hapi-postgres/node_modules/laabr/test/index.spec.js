...
test.serial.cb('listen to `response` event', (t) => {
  ...
  helpers.spawn('inject', options, injection, (log) => {
    t.truthy(log);
    t.truthy(
      log.includes('GET 127.0.0.1 http://127.0.0.1:1338/response/200 200 {}')
    );
    t.end();
  });
});
...
test.serial.cb('listen to `response` event – post', (t) => {
  ...
  helpers.spawn('inject', options, injection, (log) => {
    ...
    t.truthy(
      log.includes(
        'POST 127.0.0.1 http://127.0.0.1:1338/response/204 204 {"foo":42}'
      )
    );
    t.end();
  });
...
test.serial.cb('listen to `response` event – buffer', (t) => {
  ...
  helpers.spawn('inject', options, injection, (log) => {
    ...
    t.truthy(
      log.includes(
        'POST 127.0.0.1 http://127.0.0.1:1338/response/buffer 200 {"foo":42}'
      )
    );
    t.end();
  });
...
test.serial.cb('listen to `response` event – readable stream', (t) => {
  ...
  helpers.spawn('inject', options, injection, (log) => {
    ...
    t.truthy(
      log.includes(
        'POST 127.0.0.1 http://127.0.0.1:1338/response/stream 200 [Readable]'
      )
    );
    t.end();
  });
...
test.serial.cb(
  'listen to `log` event – concat strings – single quotes',
  (t) => {
    const options = {
      indent: 0,
      formats: { log: "{ message::message + 'bar' }" },
...
test.serial.cb(
  'listen to `log` event – inline strings – single quotes',
  (t) => {
    const options = {
      indent: 0,
      formats: { log: "{ message: [:message,'bar'] }" },
...
test.serial.cb(
  'listen to `log` event – concat strings – double quotes',
  (t) => {
    const options = {
      indent: 0,
      formats: { log: '{ message::message + "bar" }' },
...
test.serial.cb(
  'listen to `log` event – inline strings – double quotes',
  (t) => {
    const options = {
      indent: 0,
      formats: { log: '{ message: [:message,"bar"] }' },
...
test.serial.cb(
  'get log message by overriden `console.warn` – multiple',
  (t) => {
    const options = {
      indent: 0,
      override: true,
...
test.serial.cb('listen to `caught` event', (t) => {
  ...
  helpers.spawn('error', options, true, (log) => {
    ...
    t.regex(
      log.source,
      new RegExp(`^${path.join(__dirname, 'fixtures/error.js')}:`)
    );
    t.end();
  });
});
