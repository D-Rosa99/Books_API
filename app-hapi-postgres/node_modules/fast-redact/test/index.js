...
test('returns no-op when passed no paths [serialize: false]', ({
  end,
  doesNotThrow,
}) => {
...
test('returns serializer when passed no paths [serialize: default]', ({
  end,
  is,
}) => {
...
test('throws when passed non-object number using [strict: true]', ({
  end,
  throws,
}) => {
...
test('returns JSON.stringified value when passed non-object using [strict: false] and no serialize option', ({
  end,
  is,
  doesNotThrow,
}) => {
  const redactDefaultSerialize = fastRedact({
    paths: ['a.b.c'],
    strict: false,
  });
...
test('returns custom serialized value when passed non-object using [strict: false, serialize: fn]', ({
  end,
  is,
  doesNotThrow,
}) => {
...
test('returns original value when passed non-object using [strict: false, serialize: false]', ({
  end,
  is,
  doesNotThrow,
}) => {
...
test('throws if a custom serializer is used and remove is true', ({
  end,
  throws,
}) => {
  throws(() => {
...
test('redact.restore function is available when serialize is false', ({
  end,
  is,
}) => {
  const censor = 'test';
...
test('redact.restore function places original values back in place', ({
  end,
  is,
}) => {
  const censor = 'test';
...
test('masks according to supplied censor function with wildcards', ({
  end,
  is,
}) => {
  const redact = fastRedact({
...
test('masks according to supplied censor function with nested wildcards', ({
  end,
  is,
}) => {
  const redact = fastRedact({
...
test('redact.restore function places original values back in place with censor function', ({
  end,
  is,
}) => {
  const redact = fastRedact({
...
test('removes during serialization instead of redacting when remove option is true', ({
  end,
  is,
}) => {
...
test('serializes with JSON.stringify if serialize is not a function', ({
  end,
  is,
}) => {
...
test('supports paths with array indexes', ({ end, same }) => {
  ...
  same(
    redact({
      insideArray: { like: ['a', 'b', 'c', { this: { foo: 'meow' } }] },
    }),
    { insideArray: { like: ['a', 'b', 'c', { this: censor }] } }
  );
  end();
});

test('censor may be any type, including function', ({ end, same }) => {
  const redactToString = fastRedact({
    paths: ['a.b.c', 'a.b.d.*'],
    censor: 'censor',
    serialize: false,
  });
  const redactToUndefined = fastRedact({
    paths: ['a.b.c', 'a.b.d.*'],
    censor: undefined,
    serialize: false,
  });
  const sym = Symbol('sym');
  const redactToSymbol = fastRedact({
    paths: ['a.b.c', 'a.b.d.*'],
    censor: sym,
    serialize: false,
  });
  const redactToNumber = fastRedact({
    paths: ['a.b.c', 'a.b.d.*'],
    censor: 0,
    serialize: false,
  });
  const redactToBoolean = fastRedact({
    paths: ['a.b.c', 'a.b.d.*'],
    censor: false,
    serialize: false,
  });
  const redactToNull = fastRedact({
    paths: ['a.b.c', 'a.b.d.*'],
    censor: null,
    serialize: false,
  });
  const redactToObject = fastRedact({
    paths: ['a.b.c', 'a.b.d.*'],
    censor: { redacted: true },
    serialize: false,
  });
  const redactToArray = fastRedact({
    paths: ['a.b.c', 'a.b.d.*'],
    censor: ['redacted'],
    serialize: false,
  });
  const redactToBuffer = fastRedact({
    paths: ['a.b.c', 'a.b.d.*'],
    censor: Buffer.from('redacted'),
    serialize: false,
  });
  const redactToError = fastRedact({
    paths: ['a.b.c', 'a.b.d.*'],
    censor: Error('redacted'),
    serialize: false,
  });
  const redactToFunction = fastRedact({
    paths: ['a.b.c', 'a.b.d.*'],
    censor: () => 'redacted',
    serialize: false,
  });
  same(redactToString({ a: { b: { c: 's', d: { x: 's', y: 's' } } } }), {
    a: { b: { c: 'censor', d: { x: 'censor', y: 'censor' } } },
  });
  same(redactToUndefined({ a: { b: { c: 's', d: { x: 's', y: 's' } } } }), {
    a: { b: { c: undefined, d: { x: undefined, y: undefined } } },
  });
  same(redactToSymbol({ a: { b: { c: 's', d: { x: 's', y: 's' } } } }), {
    a: { b: { c: sym, d: { x: sym, y: sym } } },
  });
  same(redactToNumber({ a: { b: { c: 's', d: { x: 's', y: 's' } } } }), {
    a: { b: { c: 0, d: { x: 0, y: 0 } } },
  });
  same(redactToBoolean({ a: { b: { c: 's', d: { x: 's', y: 's' } } } }), {
    a: { b: { c: false, d: { x: false, y: false } } },
  });
  same(redactToNull({ a: { b: { c: 's', d: { x: 's', y: 's' } } } }), {
    a: { b: { c: null, d: { x: null, y: null } } },
  });
  same(redactToObject({ a: { b: { c: 's', d: { x: 's', y: 's' } } } }), {
    a: {
      b: {
        c: { redacted: true },
        d: { x: { redacted: true }, y: { redacted: true } },
      },
    },
  });
  same(redactToArray({ a: { b: { c: 's', d: { x: 's', y: 's' } } } }), {
    a: { b: { c: ['redacted'], d: { x: ['redacted'], y: ['redacted'] } } },
  });
  same(redactToBuffer({ a: { b: { c: 's', d: { x: 's', y: 's' } } } }), {
    a: {
      b: {
        c: Buffer.from('redacted'),
        d: { x: Buffer.from('redacted'), y: Buffer.from('redacted') },
      },
    },
  });
  same(redactToError({ a: { b: { c: 's', d: { x: 's', y: 's' } } } }), {
    a: {
      b: {
        c: Error('redacted'),
        d: { x: Error('redacted'), y: Error('redacted') },
      },
    },
  });
  same(redactToFunction({ a: { b: { c: 's', d: { x: 's', y: 's' } } } }), {
    a: { b: { c: 'redacted', d: { x: 'redacted', y: 'redacted' } } },
  });
  end();
});
...
test('supports strings in bracket notation paths (single quote)', ({
  end,
  is,
}) => {
...
test('supports strings in bracket notation paths (double quote)', ({
  end,
  is,
}) => {
  const redact = fastRedact({ paths: [`a["@#!="].c`], serialize: false });
...
test('supports strings in bracket notation paths (backtick quote)', ({
  end,
  is,
}) => {
  const redact = fastRedact({ paths: ['a[`@#!=`].c'], serialize: false });
...
test('gracefully handles primitives that match intermediate keys in paths', ({
  end,
  same,
}) => {
...
test('handles circulars and cross references – restore', ({
  end,
  is,
  same,
}) => {
...
test('ultimate wildcards – shallow', ({ end, same }) => {
  ...
  same(redact({ test: { baz: 1, bar: 'private' } }), {
    test: { baz: censor, bar: censor },
  });
  end();
});
...
test('ultimate wildcards – deep', ({ end, same }) => {
  ...
  same(
    redact({
      deep: {
        a: 1,
        bar: { b: 2, baz: { c: 3, ding: { d: 4, e: 5, f: 'six' } } },
      },
    }),
    {
      deep: {
        a: 1,
        bar: { b: 2, baz: { c: 3, ding: { d: censor, e: censor, f: censor } } },
      },
    }
  );
  end();
...
test('ultimate wildcards – array – deep', ({ end, same }) => {
  ...
  same(redact({ deepArray: { down: { here: ['a', 'b', 'c'] } } }), {
    deepArray: { down: { here: [censor, censor, censor] } },
  });
  end();
});
...
test('ultimate wildcards – array – single index', ({ end, same }) => {
  ...
  same(
    ...
    { insideArray: { like: ['a', 'b', 'c', { this: { foo: censor } }] } }
  );
  end();
});
...
test('ultimate wildcards - handles paths that do not match object structure', ({
  end,
  same,
}) => {
  const redact = fastRedact({ paths: ['x.y.z'], serialize: false });
...
test('ultimate wildcards - gracefully handles primitives that match intermediate keys in paths', ({
  end,
  same,
}) => {
  const redact = fastRedact({ paths: ['a.b.c', 'a.b.c.d'], serialize: false });
...
test('ultimate wildcards – handles circulars – restore', ({
  end,
  is,
  same,
}) => {
...
test('ultimate wildcards – handles circulars and cross references – restore', ({
  end,
  is,
  same,
}) => {
...
test('static + wildcards', ({ end, is }) => {
  ...
  const result = redact({
    a: { b: { c: 's', z: { x: 's', y: 's' } }, d: { a: 's', b: 's', c: 's' } },
  });
...
test('parent wildcards - handles paths that do not match object structure', ({
  end,
  same,
}) => {
...
test('parent wildcards - gracefully handles primitives that match intermediate keys in paths', ({
  end,
  same,
}) => {
  const redact = fastRedact({ paths: ['a.*.c'], serialize: false });
...
test('parent wildcards – handles circulars and cross references – restore', ({
  end,
  is,
  same,
}) => {
...
test('parent wildcards – removes during serialization instead of redacting when remove option is true', ({
  end,
  is,
}) => {
...
test('ultimate wildcards – removes during serialization instead of redacting when remove option is true', ({
  end,
  is,
}) => {
...
test('supports leading bracket notation containing non-legal keyword characters', ({
  end,
  is,
}) => {
...
test('supports single leading bracket containing non-legal keyword characters', ({
  end,
  is,
}) => {
  const censor = 'test';
...
test('(leading brackets) ultimate wildcards – handles circulars and cross references – restore', ({
  end,
  is,
  same,
}) => {
...
test('(leading brackets) parent wildcards – handles circulars and cross references – restore', ({
  end,
  is,
  same,
}) => {
...
test('(leading brackets) ultimate wildcards – handles missing paths', ({
  end,
  is,
  same,
}) => {
...
test('correctly restores original object when a path does not match object', ({
  end,
  is,
}) => {
...
test('correctly restores original object when a matching path has value of `undefined`', ({
  end,
  is,
}) => {
  const redact = fastRedact({ paths: ['foo.bar'], strict: false });
