...
async function register(server, options) {
  ...
  const getChildBindings = options.getChildBindings
    ? options.getChildBindings
    : (request) => ({ req: request });
  const shouldLogRequestStart =
    typeof options.logRequestStart === 'function'
      ? (request) => options.logRequestStart(request)
      : typeof options.logRequestStart === 'boolean'
      ? () => !!options.logRequestStart
      : () => false;
  const shouldLogRequestComplete =
    typeof options.logRequestComplete === 'function'
