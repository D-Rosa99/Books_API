...
class JoyCon {
  ...
  removeLoader(name) {
    ...
    try {
      for (
        var _iterator = this.loaders[Symbol.iterator](), _step;
  ...
  recusivelyResolve(options) {
    ...
    return _asyncToGenerator(function* () {
      ...
      try {
        for (
          var _iterator4 = options.files[Symbol.iterator](), _step4;
          ...
          const exists =
            process.env.NODE_ENV !== 'test' && _this.existsCache.has(file)
              ? _this.existsCache.get(file)
              : yield pathExists(file);
          ...
          if (exists) {
            if (
              !options.packageKey ||
              _path.default.basename(file) !== 'package.json'
            ) {
              return file;
            }
      ...
      return _this.recusivelyResolve(
  ...
  recusivelyResolveSync(options) {
    ...
    try {
      for (
        var _iterator2 = options.files[Symbol.iterator](), _step2;
  ...
  findLoader(filepath) {
    ...
    try {
      for (
        var _iterator3 = this.loaders[Symbol.iterator](), _step3;
