...
test('basic prettifier tests', (t) => {
    ...
    const log = pino(
      ...
      new Writable({
        write(chunk, enc, cb) {
          ...
          t.is(formatted, `[${epoch}] INFO  (${pid} on ${hostname}): foo\n`);
          cb();
        },
      })
    );
  ...
  t.test('can swap date and level position', (t) => {
    ...
    const log = pino(
      ...
      new Writable({
        write(chunk, enc, cb) {
          const formatted = pretty(chunk.toString());
          t.is(formatted, `INFO  [${epoch}] (${pid} on ${hostname}): foo\n`);
          cb();
        },
      })
    );
  ...
  t.test('can use different message keys', (t) => {
    ...
    const log = pino(
      ...
      new Writable({
        write(chunk, enc, cb) {
          const formatted = pretty(chunk.toString());
          t.is(formatted, `[${epoch}] INFO  (${pid} on ${hostname}): baz\n`);
          cb();
        },
      })
    );
  ...
  t.test('can use different level keys', (t) => {
    ...
    const log = pino(
      ...
      new Writable({
        write(chunk, enc, cb) {
          const formatted = pretty(chunk.toString());
          t.is(formatted, `[${epoch}] WARN  (${pid} on ${hostname}): foo\n`);
          cb();
        },
      })
    );
  ...
  t.test('handles missing name', (t) => {
    ...
    const msg = 'hello world';
    const regex = new RegExp(
      '\\[.*\\] INFO  \\(' + process.pid + ' on ' + hostname + '\\): ' + msg
    );
    ...
    const log = pino(
      ...
      new Writable({
        write(chunk, enc, cb) {
          ...
          t.is(
            formatted,
            `[${epoch}] INFO  (matteo/${pid} on ${hostname}): hello world\n`
          );
          cb();
        },
      })
  ...
  t.test('handles `undefined` return values', (t) => {
    ...
    let formatted = pretty(
      `{"msg":"nope", "time":${epoch}, "level":30, "v":1}`
    );
    t.is(formatted, undefined);
    formatted = pretty(
      `{"msg":"hello world", "time":${epoch}, "level":30, "v":1}`
    );
  ...
  t.test('prettifies custom key', (t) => {
    ...
    const arst = pretty(
      '{"msg":"hello world", "foo": "bar", "cow": "moo", "level":30, "v":1}'
    );
    t.is(
      arst,
      'INFO : hello world\n    foo: bar_baz\n    multiline\n    cow: MOO\n'
  ...
  t.test('does not prettify custom key that does not exists', (t) => {
    ...
    const arst = pretty(
      '{"msg":"hello world", "foo": "bar", "level":30, "v":1}'
    );
  ...
  t.test('ignores multiple keys', (t) => {
    ...
    const arst = pretty(
      `{"msg":"hello world", "pid":"${pid}", "hostname":"${hostname}", "time":${epoch}, "level":30, "v":1}`
    );
