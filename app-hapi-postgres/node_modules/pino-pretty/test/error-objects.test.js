...
test('error like objects tests', (t) => {
    ...
    const log = pino(
      ...
      new Writable({
        write(chunk, enc, cb) {
          ...
          t.is(
            lines[0],
            `[${epoch}] INFO  (${pid} on ${hostname}): hello world`
          );
          cb();
        },
      })
  ...
  t.test(
    'prettifies Error in property within errorLikeObjectKeys when stack has escaped characters',
    (t) => {
      t.plan(8);
      const pretty = prettyFactory({
        errorLikeObjectKeys: ['err'],
  ...
  t.test(
    'prettifies Error in property within errorLikeObjectKeys when stack is not the last property',
    (t) => {
      t.plan(9);
      const pretty = prettyFactory({
        errorLikeObjectKeys: ['err'],
