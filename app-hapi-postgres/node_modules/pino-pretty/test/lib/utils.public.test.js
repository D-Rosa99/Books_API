...
tap.test('prettifyMessage', (t) => {
  ...
  t.test(
    'returns non-colorized value for alternate `messageKey`',
    async (t) => {
  ...
  t.test(
    'returns colorized value for color colorizer for alternate `messageKey`',
    async (t) => {
      const colorizer = getColorizer(true);
      const str = prettifyMessage({
        log: { message: 'foo' },
  ...
  t.test(
    'returns message formatted by `messageFormat` option - missing prop',
    async (t) => {
      const str = prettifyMessage({
...
tap.test('prettifyMetadata', (t) => {
  ...
  t.test('works with only `name`, `pid`, & `hostname` present', async (t) => {
    const str = prettifyMetadata({
      log: { name: 'foo', pid: '1234', hostname: 'bar' },
    });
  ...
  t.test('works with only `name`, `pid`, & `caller` present', async (t) => {
    const str = prettifyMetadata({
      log: { name: 'foo', pid: '1234', caller: 'baz' },
    });
    t.is(str, '(foo/1234) <baz>');
  });

  t.test(
    'works with only `name`, `hostname`, & `caller` present',
    async (t) => {
      const str = prettifyMetadata({
        log: { name: 'foo', hostname: 'bar', caller: 'baz' },
      });
  ...
  t.test('works with only `pid`, `hostname`, & `caller` present', async (t) => {
    const str = prettifyMetadata({
      log: { pid: '1234', hostname: 'bar', caller: 'baz' },
    });
...
tap.test('prettifyTime', (t) => {
  ...
  t.test(
    'returns `undefined` if `time` or `timestamp` not in log',
    async (t) => {
