...
tap.test('#joinLinesWithIndentation', (t) => {
  t.test(
    'joinLinesWithIndentation adds indentation to beginning of subsequent lines',
    async (t) => {
      const input = 'foo\nbar\nbaz';
  ...
  t.test(
    'joinLinesWithIndentation accepts custom indentation, line breaks, and eol',
    async (t) => {
      const input = 'foo\nbar\r\nbaz';
      const result = internals.joinLinesWithIndentation({
        input,
        ident: '  ',
        eol: '^',
      });
...
tap.test('#formatTime', (t) => {
  ...
  t.test(
    'translates epoch milliseconds if `translateTime` is `true`',
    async (t) => {
  ...
  t.test(
    'translates epoch milliseconds to UTC string given format',
    async (t) => {
  ...
  t.test(
    'passes through date string if `translateTime` is `false`',
    async (t) => {
