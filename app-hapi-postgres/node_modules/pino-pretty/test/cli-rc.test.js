...
test('cli', (t) => {
  ...
  t.test(
    'loads and applies default config file: pino-pretty.config.js',
    (t) => {
      t.plan(1);
      ...
      child.stdout.on('data', (data) => {
        t.is(
          data.toString(),
          '[2018-03-30 17:35:28.992 +0000] INFO  (42 on foo): hello world\n'
        );
      });
  ...
  t.test('loads and applies default config file: .pino-prettyrc', (t) => {
    ...
    fs.writeFileSync(
      configFile,
      JSON.stringify({ translateTime: true }, null, 4)
    );
    const env = { TERM: 'dumb' };
    const child = spawn(process.argv[0], [bin], { env, cwd: tmpDir });
    // Validate that the time has been translated
  ...
  t.test(
    'loads and applies custom config file: pino-pretty.config.test.json',
    (t) => {
      t.plan(1);
      // Set translateTime: true on run configuration
      ...
      const env = { TERM: 'dumb' };
      const child = spawn(process.argv[0], [bin, '--config', configFile], {
        env,
        cwd: tmpDir,
      });
      // Validate that the time has been translated
  ...
  t.test(
    'loads and applies custom config file: pino-pretty.config.test.js',
    (t) => {
      t.plan(1);
      // Set translateTime: true on run configuration
  ...
  t.test('cli options override config options', (t) => {
    ...
    const env = { TERM: 'dumb' };
    const child = spawn(process.argv[0], [bin, '--messageKey', 'new_msg'], {
      env,
      cwd: tmpDir,
    });
  ...
  t.test('throws on invalid default config file', (t) => {
    ...
    child.stderr.on('data', (data) => {
      t.contains(
        data.toString(),
        'Error: Invalid runtime configuration file: pino-pretty.config.js\n'
      );
    });
    t.tearDown(() => child.kill());
  });
